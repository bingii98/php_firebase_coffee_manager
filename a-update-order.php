    ...
    <table cellpadding="10" cellspacing="1" style="width: 100%; text-align: left; font-size: 14px;">
        ...
        foreach ($table->getOrders() as $key => $item) {
            ...
            if (!$isFN_detail) { ?>
                <tr>
                    <td colspan="5" align=right>
                        <button class="btn btn-primary btn-update-order" data_order="<?php echo $item->getId() ?>" data_table="<?php echo $table->getId() ?>"><i class="fa fa-paper-plane" aria-hidden="true"></i> đã thực hiện</button>
                    </td>
                </tr>
            <?php }
        }
